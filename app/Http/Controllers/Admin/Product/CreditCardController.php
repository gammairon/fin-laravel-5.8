...
class CreditCardController extends BaseAdminController
{
    protected $service;
    ...
    public function index(Request $request)
    {
        ...
        $cards = $query->paginate(config('settings.perPage'));

        return view('admin.product.credit_card.list', compact('cards'));
    }
    ...
    public function create()
    {
        ...
        $selfWithdrawalFees = [''];
        $foreignWithdrawalFees = [''];
        $additionalFees = [''];
        $documents = [''];
        $seo = new Seo();

        return view('admin.product.credit_card.create', compact('creditCard', 'banks', 'seo', 'selfWithdrawalFees', 'foreignWithdrawalFees', 'additionalFees', 'documents'));
    }
    ...
    {
        $banks = Bank::all()->pluck('name', 'id');
        $selfWithdrawalFees = $creditCard->getSelfWithdrawalFees();
        $foreignWithdrawalFees = $creditCard->getForeignWithdrawalFees();
        $additionalFees = $creditCard->getAdditionalFees();
        $documents = $creditCard->getDocuments();
        $seo = $creditCard->seo ?: new Seo();
        return view('admin.product.credit_card.edit', compact('creditCard', 'banks', 'seo', 'selfWithdrawalFees', 'foreignWithdrawalFees', 'additionalFees', 'documents'));
    }
    ...
    public function update(CreditCardRequest $request, CreditCard $creditCard)
    {
        ...
        if ($creditCard = $this->service->update($creditCard, $request->all()))
            $msg = ['success' => 'Продукт был удачно обновлен!'];

        return redirect()->route('admin.products.credit-cards.edit', $creditCard)->with($msg);
    }
