...
trait SeoServiceTrait
{
    ...
    public function setSeo(Seo $seo, $model = null)
    {
        ...
        $this->seoRegex += ['%%sitename%%' => Setting::getSiteName(), '%%sitedesc%%' => Setting::getSiteDescription()];
    ...
    protected function addSeoRegex(array $values)
    {
        $this->seoRegex += $values;
    }

    //Заменяем все регулярки строками
    protected function replaceRegex(?string $value): string
    {
        return str_replace(array_keys($this->seoRegex), array_values($this->seoRegex), $value);
    }
