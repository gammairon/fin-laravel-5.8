...
trait MediaTrait
{
    ...
    {
        $this->addMediaConversion('thumb')->width(300)->height(200)->format('webp');

        /*$this->addMediaConversion('middle')->width(650)->height(350)->format('webp');

        $this->addMediaConversion('sidebar')->width(400)->height(200)->format('webp');*/
    }
    ...
    {
        if(file_exists($file)){
            $currentMedia = $this->getMedia($collectionName)->first();

            if($currentMedia && basename($file, PATHINFO_BASENAME) == basename($currentMedia->getPath(), PATHINFO_BASENAME))
                return;

            $this->addMedia( $file )->preservingOriginal()->withCustomProperties($properties)->toMediaCollection($collectionName);
        }
