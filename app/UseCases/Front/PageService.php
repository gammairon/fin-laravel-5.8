...
class PageService extends BaseService
{
    ...
    public function getView($slug)
    {
        ...
        if($page = $this->getPage($slug)){
            ...
            return view('front.blog.page', compact('page'))->compileShortcodes();
        }


        if($post = $this->getPost($slug)){
            $posts = Cache::tags(['posts'])->remember('single:posts:'.$post->id, 21600, function () use ($post){
                return Post::with(['categories', 'user', 'tags', 'media', 'seo', 'faqs'])
                    ->where('id', '<>', $post->id)
            ...
            return view('front.blog.post', compact('post', 'posts', 'loadedPosts'))->compileShortcodes();
        }
    ...
    public function getPost(string $slug): ?Post
    {
        ...
        return Cache::tags(['posts'])->remember('single:post:'.$post->id, 21600, function () use ($post){
            return $post->load(['categories', 'user', 'tags', 'media', 'seo', 'faqs']);
        });
    }
