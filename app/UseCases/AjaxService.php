...
class AjaxService extends BaseService
{
    ...
    public function mfoFilter(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if($no_procent = $request->input('no_procent')){
                $builder->where('free_credit_bid', '==', 0);
            }

            if($age_from = $request->input('age_from')){
                $builder->where('min_age', '>=', 18);
            }

            if($age_to = $request->input('age_to')){
                $builder->where('max_age', '<=', 80);
            }

            $mfos = $builder->sort($request->input('order'))->get();
        }
    ...
    public function loadMorePosts(Request $request)
    {
        ...
        $query = Post::whereNotIn('id', array_keys($except))->public()->with(['categories', 'user', 'tags', 'media', 'seo'])->limit(2);
        ...
        $posts = Cache::tags(['posts'])->remember('load_more:posts:'.$lastPostId, 21600, function () use($query){
            return $query->get();
        });
        ...
        return response()->json([
            'content' =>  view('front.partials.blog.list_single_post', compact('posts'))->render(),
            'except' => $except,
        ]);
    }
    ...
    public  function creditCashFilter(Request $request)
    {
        ...
        $query = CreditCash::with(['bank', 'media'])->orderByDesc('max_amount');
        ...
        if (!$validator->fails()) {
            ...
            if($month = $request->input('month'))
                $query->where('max_term', '>=', $month)->where('min_term', '<=', $month);
        }

        $creditsCash = $query->get();

        return view('front.partials.product.credit_cash_list', ['creditsCash' => $creditsCash]);
    }
