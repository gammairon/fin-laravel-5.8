          ...
          <table id="credit_card_list" class="table table-bordred table-striped">
            ...
            <tbody>
              @foreach ($cards as $card)
                 <tr>
                  <td><input type="checkbox" form="all-action" class="checkthis" name="ids[]" value="{{$card->id}}" /></td>
                    <td>{{$card->name}}</td>
                    <td>{{$card->slug}}</td>
                    <td>
                      <a href="{{ route('admin.products.credit-cards.edit', $card) }}" class="btn btn-primary btn-xs" data-title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                    </td>
                    <td>
                      <a href="javascript:void(0);" class="btn btn-danger btn-xs btn-delete" data-title="Delete" data-form="#form-delete-{{$card->id}}"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                      {!! Form::open([
                          'method' => 'DELETE',
                          'route' => ['admin.products.credit-cards.destroy', $card->id],
