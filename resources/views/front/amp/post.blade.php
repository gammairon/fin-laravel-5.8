    ...
    <div class="single-post">
            ...
            <ul class="post-categories">
                @if ($post->categories)
                    @foreach ($post->categories as $category)
                        <li><a href="{{ route('category', $category) }}" title="{{$category->name}}">{{$category->name}}</a></li>
                    @endforeach
                @endif
            </ul>
        ...
        <div class="post-content">
            ...
            <div class="img-detail-post">
                <amp-img src="{{url($post->getPrimary()->getUrl('thumb'))}}" alt="{{$post->getPrimaryAlt()}}" title="{{$post->getPrimaryTitle()}}" width="650" height="350" layout="responsive" ></amp-img>
            </div>
            {!!$ampContent!!}
        </div>
        <div class="post-footer">
            ...
            <div class="popular-posts">
                ...
                <ul class="list-popular-post">
                    @if ($popularPosts = $post->getPopularPostsByTags())
                        @foreach ($popularPosts as $popularPost)
                            <li class="popular-post-item">

                                <div class="img-detail-news">
                                    <a href="{{ route('page', ['slug' => $popularPost->slug]) }}" class="popular-post-img" >
                                        <amp-img src="{{url($popularPost->getPrimary()->getUrl('thumb'))}}" alt="dssdsd" title="sdsdsd" width="650" height="350" layout="responsive" ></amp-img>
                                    </a>
                                </div>
